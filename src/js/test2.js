...
window.onload = function() {
  ...
  function objectToLocalStorage(toDoList){
    for (let i = 0; i < toDoList.length; i++) {
    let myObj = JSON.stringify(toDoList);
    //console.log("string",myObj);
    localStorage.setItem("new todo", myObj);
    localStorageBackToObj(myObj)
      
    }
    
  }
function localStorageBackToObj(myObj) {
  localStorage.getItem("new todo", myObj);
  let stringToObject = JSON.parse(myObj);
  toDoList.push(stringToObject);
  console.log("obj: ", stringToObject)
  createList(stringToObject);
}
//localStorage.removeItem("new todo");
 
  function createList(toDofromStorage){
  ...
  for (i=0;i<toDofromStorage.length;i++){
    ...
    newLi.innerHTML+=toDofromStorage[i].things;
    let theList = toDofromStorage[i]; //skapar en variabel till min lista som loopas
  ...
  function addToList() { //sparar value från input och gör om den till false. Vet dock inte hur jag ska få in den i min klass.
    ...
    //objectToLocalStorage(BackToList)
